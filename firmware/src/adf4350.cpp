...
ADF4350::ADF4350(byte LEPin, byte ADC_pin) {
    _LEPin = LEPin;
    _ADCpin = ADC_pin;
    pinMode(_LEPin, OUTPUT);
    digitalWrite(_LEPin, HIGH);
    // setup ESP32 SPI object
...
void ADF4350::init(int freq, int refClk = 10){
    _refClk = refClk;
    _phase = 1;
...
void ADF4350::send_2870(){
  // mamually calculated
  byte dataArr[6][5] = {{5, 0x00, 0x58, 0x00, 0x05},
                        {4, 0x00, 0x86, 0x43, 0x24},
                        {3, 0x00, 0x04, 0x00, 0x03},
                        {2, 0x20, 0x00, 0xBE, 0x42},
...
int ADF4350::send_sweep_step(int i){
    for(int j = 0; j < 6; j++){
      if (sweepArr[i][j][0] == 6) {
        continue; // skip this register...
      }
